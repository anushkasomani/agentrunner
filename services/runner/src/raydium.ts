...
export async function raydiumSwap(params: {
  ...
  amount: number
  slippageBps: number
}): Promise<{ txid?: string; outAmount?: string; error?: string }> {
  try {
    ...
    let priorityData: SwapCompute
    try {
      const response = await axios.get<SwapCompute>(priorityUrl)
    ...
    const { data: swapResponse } = await axios.get<SwapCompute>(quoteUrl)
    ...
    let inputTokenAcc: string | undefined
    let outputTokenAcc: string | undefined
    ...
    const { data: swapTransactions } = await axios.post<SwapResponse>(
 ...
 */
export async function getTokenAccount(mint: string): Promise<string | null> {
  try {
    const tokenAccounts = await connection.getTokenAccountsByOwner(owner.publicKey, {
 ...
 */
export async function checkBalance(mint: string, requiredAmount: number): Promise<boolean> {
  try {
