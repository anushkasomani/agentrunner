...
const PAYTO_MINT = new PublicKey(process.env.PAYTO_MINT || "4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU"); // USDC mint
const PAYTO_ADDRESS = new PublicKey(process.env.PAYTO_ADDRESS || "HNMhpZQuQ3aJ1ePix4Q8afwUxDFmGNC4ReknNgFmNbq3"); // merchant USDC token account (from x402-merchant)
export const PAYER_SECRET_KEY=process.env.PAYER_SECRET_KEY || [94,21,75,106,120,153,168,235,114,39,104,0,255,148,42,122,107,26,9,2,228,10,81,11,232,159,190,211,236,16,59,232,243,51,153,0,152,19,129,196,31,240,193,61,248,14,75,207,158,187,213,67,243,131,40,248,248,198,180,155,152,217,219,178]
const PAYER_PRIV = Uint8Array.from(PAYER_SECRET_KEY as any);
...
export async function makePayment(invoice: string): Promise<any> {
    ...
    try {
        ...
        const merchantPubkey = new PublicKey(process.env.MERCHANT_PUBLIC_KEY || "51j3b8cZkYwAeKA47rEGWs8vLm12RD82yAgHhYYhyimr");
